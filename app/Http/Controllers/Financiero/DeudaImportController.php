...
class DeudaImportController extends Controller
{
    ...
    public function execute(Request $request)
    {
        ...
        try {
            ...
            // TRUNCATE the table (fast, resets sequences)
            ...
            foreach ($chunks as $chunk) {
                ...
                try {
                    DB::table('cond_deudas_apto')->insert($inserts);
                    $results['imported'] += count($inserts);
                } catch (\Exception $e) {
                    // If chunk fails, try one by one
                    foreach ($inserts as $insert) {
                        try {
                            DB::table('cond_deudas_apto')->insert($insert);
                            $results['imported']++;
                        } catch (\Exception $e2) {
                            $results['errors'][] = [
                                'info' => ($insert['cod_edif_legacy'] ?? '') . '/' . ($insert['num_apto_legacy'] ?? '') . '/' . ($insert['periodo'] ?? ''),
            ...
            @unlink($tempPath);
            return redirect()->route('financiero.deudas.importar')
                ->with('error', 'Error critico en la importacion: ' . $e->getMessage());
        }
