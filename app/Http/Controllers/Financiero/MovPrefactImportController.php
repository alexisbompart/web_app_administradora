    ...
    public function execute(Request $request)
    {
        ...
        try {
            ...
            foreach (array_chunk($rows, 500) as $chunk) {
                ...
                try {
                    DB::table('cond_movimientos_prefact')->insert($inserts);
                    $results['imported'] += count($inserts);
                } catch (\Exception $e) {
                    foreach ($inserts as $insert) {
                        try {
                            DB::table('cond_movimientos_prefact')->insert($insert);
                            $results['imported']++;
                        } catch (\Exception $e2) {
                            $results['errors'][] = [
                                'info' => ($insert['cod_edif_legacy'] ?? '') . '/' . ($insert['num_apto_legacy'] ?? '') . '/' . ($insert['cod_gasto_legacy'] ?? ''),
            ...
            DB::rollBack();
            @unlink($tempPath);
            return redirect()->route('financiero.movprefact.importar')->with('error', 'Error critico: ' . $e->getMessage());
        }
