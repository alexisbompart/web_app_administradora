    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate_custom-400 mt-1">cond_pagos</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('financiero.pagos.importar') }}" class="btn-primary">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="text-sm font-heading font-semibold text-navy-800">
                <i class="fas fa-money-check-alt mr-2 text-burgundy-800"></i>Listado
            </h3>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="table-custom">
                    ...
                    <tbody>
                        ...
                        <tr>
                                <td class="text-xs font-semibold">{{ $item->id_pago_legacy ?? $item->numero_recibo }}</td>
                                <td class="text-xs">{{ $item->cajero }}</td>
                                <td class="text-xs">{{ $item->fecha_pago?->format('d/m/Y') }}</td>
                                <td class="text-xs font-semibold">{{ number_format($item->monto_total ?? 0, 2, ',', '.') }}</td>
                                <td class="text-xs">{{ $item->tipo_pago ?? $item->forma_pago }}</td>
                                <td class="text-xs">{{ $item->estatus }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
