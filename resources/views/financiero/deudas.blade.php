    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate_custom-400 mt-1">cond_deudas_apto</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('financiero.deudas.importar') }}" class="btn-primary">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="text-sm font-heading font-semibold text-navy-800">
                <i class="fas fa-file-invoice-dollar mr-2 text-burgundy-800"></i>Listado
            </h3>
        </div>
                ...
                <table class="table-custom">
                    ...
                    <tbody>
                        ...
                        <tr>
                                ...
                                <td class="text-xs font-semibold">{{ number_format($item->monto_original ?? 0, 2, ',', '.') }}</td>
                                <td class="text-xs font-semibold text-red-600">{{ number_format($item->saldo ?? 0, 2, ',', '.') }}</td>
                                <td>
                                    @if($item->estatus === 'P')<span class="badge-warning text-xs">Pendiente</span>
                                    @elseif($item->estatus === 'C')<span class="badge-success text-xs">Cancelada</span>
                                    @else<span class="badge-danger text-xs">{{ $item->estatus }}</span>@endif
                                </td>
                        </tr>
                        @endforeach
                    </tbody>
