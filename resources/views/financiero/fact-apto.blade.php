    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate_custom-400 mt-1">cond_movs_fact_apto</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('financiero.movfactapto.importar') }}" class="btn-primary">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="text-sm font-heading font-semibold text-navy-800">
                <i class="fas fa-file-invoice mr-2 text-burgundy-800"></i>Listado
            </h3>
        </div>
                ...
                <table class="table-custom">
                    ...
                    <tbody>
                        ...
                        <tr>
                                ...
                                <td class="text-xs">{{ $item->nombre_propietario }}</td>
                                <td class="text-xs font-semibold">{{ number_format($item->pago_total ?? 0, 2, ',', '.') }}</td>
                                <td class="text-xs">{{ $item->fecha_fact?->format('d/m/Y') }}</td>
                                <td class="text-xs">{{ $item->mes_deuda }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
