<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-heading font-bold text-navy-800">Importar Pagos</h2>
                <p class="text-sm text-slate_custom-400 mt-1">Carga completa — reemplaza todos los pagos</p>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl flex items-center gap-2"><i class="fas fa-check-circle"></i>Carga completada. {{ number_format($results['previous_count']) }} eliminados, {{ number_format($results['imported']) }} importados.</div>
        ...
        <div class="card" x-data="{ show: false }">
            ...
            <div class="card-body p-0" x-show="show" x-transition><div class="overflow-x-auto max-h-64 overflow-y-auto"><table class="table-custom"><thead><tr><th>ID Pago</th><th>Razon</th></tr></thead><tbody>@foreach(array_slice($results['errors'], 0, 100) as $err)<tr><td class="text-xs font-medium">{{ $err['info'] }}</td><td class="text-red-600 text-xs">{{ \Illuminate\Support\Str::limit($err['reason'], 100) }}</td></tr>@endforeach</tbody></table></div></div>
        </div>
        @endif
        <div class="flex gap-3">
            <a href="{{ route('financiero.cobranza.index') }}" class="btn-primary"><i class="fas fa-hand-holding-usd mr-2"></i>Cobranza</a>
            <a href="{{ route('financiero.pagos.importar') }}" class="btn-secondary"><i class="fas fa-file-import mr-2"></i>Nueva Carga</a>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="bg-amber-50 border border-amber-300 text-amber-800 px-5 py-4 rounded-xl">
            <div class="flex items-start gap-3"><i class="fas fa-exclamation-triangle text-xl mt-0.5"></i><div><p class="font-heading font-bold">Carga Completa — Se reemplazaran TODOS los pagos</p><p class="text-sm mt-1">Se eliminaran <strong>{{ number_format($summary['total_actual_bd']) }}</strong> y se insertaran <strong>{{ number_format($summary['validas']) }}</strong> nuevos.</p></div></div>
        </div>
        ...
        @if(count($errors) > 0)
        <div class="card" x-data="{ show: false }">
            <div class="card-header cursor-pointer" @click="show = !show"><h3 class="text-sm font-heading font-semibold text-red-600 flex items-center justify-between w-full"><span><i class="fas fa-exclamation-triangle mr-2"></i>Errores ({{ count($errors) }})</span><i class="fas" :class="show ? 'fa-chevron-up' : 'fa-chevron-down'"></i></h3></div>
            <div class="card-body p-0" x-show="show" x-transition><div class="overflow-x-auto max-h-64 overflow-y-auto"><table class="table-custom"><thead><tr><th>Linea</th><th>Info</th><th>Razon</th></tr></thead><tbody>@foreach(array_slice($errors, 0, 50) as $err)<tr><td>{{ $err['line'] }}</td><td class="text-xs">{{ $err['info'] }}</td><td class="text-red-600 text-xs">{{ $err['reason'] }}</td></tr>@endforeach</tbody></table></div></div>
        </div>
        @endif
        <div class="card">
            <div class="card-header"><h3 class="text-sm font-heading font-semibold text-navy-800"><i class="fas fa-eye mr-2 text-burgundy-800"></i>Muestra ({{ count($previewRows) }})</h3></div>
            <div class="card-body p-0"><div class="overflow-x-auto"><table class="table-custom"><thead><tr><th>Linea</th><th>ID Pago</th><th>Cajero</th><th>Monto</th><th>Fecha</th><th>Tipo</th><th>Estatus</th></tr></thead><tbody>
                @foreach($previewRows as $row)
                <tr><td class="text-xs">{{ $row['line'] }}</td><td class="font-medium text-sm">{{ $row['display']['id_pago'] }}</td><td class="text-xs">{{ $row['display']['cajero'] }}</td><td class="text-xs font-semibold">{{ $row['display']['monto'] }}</td><td class="text-xs">{{ $row['display']['fecha_pago'] }}</td><td class="text-xs">{{ $row['display']['tipo_pago'] }}</td><td class="text-xs">{{ $row['display']['estatus'] }}</td></tr>
                @endforeach
            </tbody></table></div></div>
        </div>
        @if($summary['validas'] > 0)
        <form action="{{ route('financiero.pagos.importar.execute') }}" method="POST" onsubmit="return confirm('Reemplazar {{ number_format($summary['total_actual_bd']) }} con {{ number_format($summary['validas']) }} nuevos?')">
            @csrf
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('financiero.pagos.importar') }}" class="btn-secondary"><i class="fas fa-times mr-2"></i>Cancelar</a>
                <button type="submit" class="inline-flex items-center px-5 py-2.5 bg-red-700 text-white text-sm font-heading font-semibold rounded-lg hover:bg-red-800 transition" onclick="this.disabled=true; this.innerHTML='<i class=\'fas fa-spinner fa-spin mr-2\'></i>Procesando...'; this.form.submit();"><i class="fas fa-sync-alt mr-2"></i>Reemplazar ({{ number_format($summary['validas']) }})</button>
            </div>
        </form>
        @endif
    </div>
    ...
    <div class="space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="stat-card"><div class="flex items-center justify-between"><div class="stat-label">Pagos Actuales</div><div class="w-10 h-10 bg-navy-800/10 rounded-lg flex items-center justify-center"><i class="fas fa-database text-navy-800"></i></div></div><div class="stat-value">{{ number_format($totalActual ?? 0) }}</div></div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('financiero.pagos.importar.preview') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="max-w-xl mx-auto text-center">
                        <div class="w-16 h-16 bg-burgundy-800/10 rounded-2xl flex items-center justify-center mx-auto mb-4"><i class="fas fa-money-check-alt text-2xl text-burgundy-800"></i></div>
                        <h4 class="text-lg font-heading font-bold text-navy-800 mb-2">Pagos (cond_pagos)</h4>
                        <p class="text-sm text-slate_custom-400 mb-2">Reemplaza <strong class="text-red-600">todos los pagos actuales</strong>.</p>
                        <p class="text-sm text-slate_custom-400 mb-6">Formato pipe-delimited. Max 100MB.</p>
                        <div class="mb-6">
