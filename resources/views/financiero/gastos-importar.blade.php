<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-heading font-bold text-navy-800">Importar Gastos</h2>
                <p class="text-sm text-slate_custom-400 mt-1">Catalogo de gastos — inserta nuevos o actualiza existentes</p>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl flex items-center gap-2"><i class="fas fa-check-circle"></i>Importacion completada. {{ $results['imported'] }} nuevos, {{ $results['updated'] }} actualizados, {{ $results['skipped'] }} omitidos.</div>
    ...
    <div class="space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="stat-card"><div class="flex items-center justify-between"><div class="stat-label">Nuevos</div><div class="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center"><i class="fas fa-plus-circle text-green-600"></i></div></div><div class="stat-value text-green-600">{{ number_format($summary['new']) }}</div></div>
        ...
        <form action="{{ route('financiero.gastos.importar.execute') }}" method="POST">
            @csrf
            <div class="card">
                ...
                <div class="card-body">
                    @if($summary['update'] > 0)
                    <div class="mb-6">
                        <p class="text-sm font-semibold text-navy-800 mb-3">{{ number_format($summary['update']) }} gastos ya existen. Que desea hacer?</p>
                        <div class="flex flex-col sm:flex-row gap-4">
                            <label class="flex items-center gap-3 p-4 border border-slate_custom-200 rounded-xl cursor-pointer hover:bg-slate_custom-50 transition"><input type="radio" name="duplicate_action" value="update" checked class="text-burgundy-800 focus:ring-burgundy-800"><div><p class="text-sm font-semibold text-navy-800">Actualizar existentes</p><p class="text-xs text-slate_custom-400">Sobreescribir datos</p></div></label>
                            <label class="flex items-center gap-3 p-4 border border-slate_custom-200 rounded-xl cursor-pointer hover:bg-slate_custom-50 transition"><input type="radio" name="duplicate_action" value="skip" class="text-burgundy-800 focus:ring-burgundy-800"><div><p class="text-sm font-semibold text-navy-800">Omitir existentes</p><p class="text-xs text-slate_custom-400">Solo importar nuevos</p></div></label>
                        </div>
                    </div>
                    @else
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate_custom-200">
                        <a href="{{ route('financiero.gastos.importar') }}" class="btn-secondary"><i class="fas fa-times mr-2"></i>Cancelar</a>
                        <button type="submit" class="btn-primary" onclick="this.disabled=true; this.innerHTML='<i class=\'fas fa-spinner fa-spin mr-2\'></i>Importando...'; this.form.submit();"><i class="fas fa-upload mr-2"></i>Ejecutar Importacion ({{ number_format($summary['validas']) }})</button>
                    </div>
                </div>
            </div>
        </form>
    ...
    <div class="space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="stat-card"><div class="flex items-center justify-between"><div class="stat-label">Gastos Actuales</div><div class="w-10 h-10 bg-navy-800/10 rounded-lg flex items-center justify-center"><i class="fas fa-database text-navy-800"></i></div></div><div class="stat-value">{{ number_format($totalActual ?? 0) }}</div></div>
        ...
        <div class="card">
            <div class="card-header"><h3 class="text-sm font-heading font-semibold text-navy-800"><i class="fas fa-file-import mr-2 text-burgundy-800"></i>Subir Archivo de Gastos</h3></div>
            <div class="card-body">
                <form action="{{ route('financiero.gastos.importar.preview') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="max-w-xl mx-auto text-center">
                        <div class="w-16 h-16 bg-burgundy-800/10 rounded-2xl flex items-center justify-center mx-auto mb-4"><i class="fas fa-receipt text-2xl text-burgundy-800"></i></div>
                        <h4 class="text-lg font-heading font-bold text-navy-800 mb-2">Catalogo de Gastos</h4>
                        <p class="text-sm text-slate_custom-400 mb-2">Importa nuevos gastos o actualiza existentes por <strong>COD_GASTO</strong>.</p>
