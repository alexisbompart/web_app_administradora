<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-heading font-bold text-navy-800">Importar Pagos por Apartamento</h2>
                <p class="text-sm text-slate_custom-400 mt-1">Carga completa — reemplaza todos los pagos por apartamento</p>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('financiero.pagoapto.importar') }}" class="btn-secondary"><i class="fas fa-file-import mr-2"></i>Nueva Carga</a>
        </div>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="card">
            ...
            <div class="card-body p-0"><div class="overflow-x-auto"><table class="table-custom"><thead><tr><th>Linea</th><th>COD_EDIF</th><th>NUM_APTO</th><th>Monto</th><th>Fecha Pago</th><th>ID Pago</th><th>Cajero</th></tr></thead><tbody>
                @foreach($previewRows as $row)
                <tr><td class="text-xs">{{ $row['line'] }}</td><td class="text-xs">{{ $row['display']['cod_edif'] }}</td><td class="font-medium text-sm">{{ $row['display']['num_apto'] }}</td><td class="text-xs font-semibold">{{ $row['display']['monto'] }}</td><td class="text-xs">{{ $row['display']['fecha_pag'] }}</td><td class="text-xs">{{ $row['display']['id_pago'] }}</td><td class="text-xs">{{ $row['display']['cajero'] }}</td></tr>
                @endforeach
            </tbody></table></div></div>
        </div>
        @if($summary['validas'] > 0)
        <form action="{{ route('financiero.pagoapto.importar.execute') }}" method="POST" onsubmit="return confirm('Reemplazar {{ number_format($summary['total_actual_bd']) }} con {{ number_format($summary['validas']) }} nuevos?')">
            @csrf
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('financiero.pagoapto.importar') }}" class="btn-secondary"><i class="fas fa-times mr-2"></i>Cancelar</a>
    ...
    <div class="space-y-6">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('financiero.pagoapto.importar.preview') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="max-w-xl mx-auto text-center">
                        <div class="w-16 h-16 bg-burgundy-800/10 rounded-2xl flex items-center justify-center mx-auto mb-4"><i class="fas fa-credit-card text-2xl text-burgundy-800"></i></div>
                        <h4 class="text-lg font-heading font-bold text-navy-800 mb-2">Pagos por Apartamento</h4>
