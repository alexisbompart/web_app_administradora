<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate_custom-400 mt-1">Carga completa — reemplaza todos los movimientos existentes</p>
            </div>
    ...
    <div class="space-y-6">
        ...
        <form action="{{ route('financiero.movprefact.importar.execute') }}" method="POST" onsubmit="return confirm('Se eliminaran {{ number_format($summary['total_actual_bd']) }} movimientos actuales y se insertaran {{ number_format($summary['validas']) }} nuevos. Continuar?')">
            @csrf
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('financiero.movprefact.importar') }}" class="btn-secondary"><i class="fas fa-times mr-2"></i>Cancelar</a>
                <button type="submit" class="btn-primary" onclick="this.disabled=true; this.innerHTML='<i class=\'fas fa-spinner fa-spin mr-2\'></i>Procesando...'; this.form.submit();"><i class="fas fa-sync-alt mr-2"></i>Reemplazar Movimientos ({{ number_format($summary['validas']) }})</button>
            </div>
        </form>
        @endif
    </div>
            ...
            <div class="card-body">
                <form action="{{ route('financiero.movprefact.importar.preview') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="max-w-xl mx-auto text-center">
                        ...
                        <p class="text-sm text-slate_custom-400 mb-2">Reemplaza <strong class="text-red-600">todos los movimientos actuales</strong> con los del archivo.</p>
