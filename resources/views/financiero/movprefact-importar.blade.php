<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-heading font-bold text-navy-800">Importar Movimientos Pre-facturacion</h2>
    ...
    <div class="space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="stat-card"><div class="flex items-center justify-between"><div class="stat-label">Existentes Antes</div><div class="w-10 h-10 bg-slate_custom-100 rounded-lg flex items-center justify-center"><i class="fas fa-database text-slate_custom-500"></i></div></div><div class="stat-value text-slate_custom-500">{{ number_format($results['previous_count']) }}</div><p class="text-xs text-slate_custom-400 mt-1">Conservados</p></div>
            <div class="stat-card"><div class="flex items-center justify-between"><div class="stat-label">Nuevos Agregados</div><div class="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center"><i class="fas fa-plus-circle text-green-600"></i></div></div><div class="stat-value text-green-600">{{ number_format($results['imported']) }}</div></div>
        ...
        <div class="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl flex items-center gap-2"><i class="fas fa-check-circle"></i>Carga incremental completada. {{ number_format($results['imported']) }} nuevos movimientos agregados. Total en BD: {{ number_format($results['previous_count'] + $results['imported']) }}.</div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('financiero.movprefact.importar') }}" class="btn-secondary"><i class="fas fa-file-import mr-2"></i>Nueva Carga</a>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="bg-blue-50 border border-blue-300 text-blue-800 px-5 py-4 rounded-xl">
            <div class="flex items-start gap-3"><i class="fas fa-info-circle text-xl mt-0.5"></i><div><p class="font-heading font-bold">Carga Incremental — Se agregaran nuevos movimientos</p><p class="text-sm mt-1">Se mantendran los <strong>{{ number_format($summary['total_actual_bd']) }}</strong> movimientos actuales y se agregaran <strong>{{ number_format($summary['validas']) }}</strong> nuevos.</p></div></div>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body p-0"><div class="overflow-x-auto"><table class="table-custom"><thead><tr><th>Linea</th><th>COD_EDIF</th><th>NUM_APTO</th><th>COD_GASTO</th><th>Monto</th><th>Origen</th><th>MOV_ID</th></tr></thead><tbody>
                @foreach($previewRows as $row)
                <tr><td class="text-xs">{{ $row['line'] }}</td><td class="text-xs">{{ $row['display']['cod_edif'] }}</td><td class="text-xs">{{ $row['display']['num_apto'] }}</td><td class="font-medium text-sm">{{ $row['display']['cod_gasto'] }}</td><td class="text-xs font-semibold">{{ $row['display']['monto'] }}</td><td class="text-xs">{{ $row['display']['origen'] }}</td><td class="text-xs">{{ $row['display']['mov_id'] }}</td></tr>
                @endforeach
            </tbody></table></div></div>
        </div>
        @if($summary['validas'] > 0)
        <form action="{{ route('financiero.movprefact.importar.execute') }}" method="POST" onsubmit="return confirm('Se agregaran {{ number_format($summary['validas']) }} nuevos movimientos a los {{ number_format($summary['total_actual_bd']) }} existentes. Continuar?')">
            @csrf
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('financiero.movprefact.importar') }}" class="btn-secondary"><i class="fas fa-times mr-2"></i>Cancelar</a>
                <button type="submit" class="btn-primary" onclick="this.disabled=true; this.innerHTML='<i class=\'fas fa-spinner fa-spin mr-2\'></i>Procesando...'; this.form.submit();"><i class="fas fa-plus-circle mr-2"></i>Agregar Movimientos ({{ number_format($summary['validas']) }})</button>
            </div>
        </form>
        @endif
    </div>
    ...
    <div class="space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="stat-card"><div class="flex items-center justify-between"><div class="stat-label">Movimientos Actuales</div><div class="w-10 h-10 bg-navy-800/10 rounded-lg flex items-center justify-center"><i class="fas fa-database text-navy-800"></i></div></div><div class="stat-value">{{ number_format($totalActual ?? 0) }}</div></div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('financiero.movprefact.importar.preview') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="max-w-xl mx-auto text-center">
                        <div class="w-16 h-16 bg-burgundy-800/10 rounded-2xl flex items-center justify-center mx-auto mb-4"><i class="fas fa-exchange-alt text-2xl text-burgundy-800"></i></div>
                        <h4 class="text-lg font-heading font-bold text-navy-800 mb-2">Movimientos Pre-facturacion</h4>
                        <p class="text-sm text-slate_custom-400 mb-2">Agrega <strong class="text-blue-600">nuevos movimientos</strong> sin eliminar los existentes.</p>
