    ...
    <div class="space-y-6">
        ...
        <form action="{{ route('financiero.movprefact.importar.execute') }}" method="POST" onsubmit="return confirm('Se eliminaran {{ number_format($summary['total_actual_bd']) }} movimientos actuales y se insertaran {{ number_format($summary['validas']) }} nuevos. Continuar?')">
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <button type="submit" class="inline-flex items-center px-5 py-2.5 bg-red-700 text-white text-sm font-heading font-semibold rounded-lg hover:bg-red-800 transition" onclick="this.disabled=true; this.innerHTML='<i class=\'fas fa-spinner fa-spin mr-2\'></i>Procesando...'; this.form.submit();"><i class="fas fa-sync-alt mr-2"></i>Reemplazar Movimientos ({{ number_format($summary['validas']) }})</button>
            </div>
        </form>
        @endif
    </div>
