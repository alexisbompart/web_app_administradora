<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('financiero.movfactapto.importar') }}" class="btn-secondary"><i class="fas fa-file-import mr-2"></i>Nueva Carga</a>
        </div>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="card">
            ...
            <div class="card-body p-0"><div class="overflow-x-auto"><table class="table-custom"><thead><tr><th>Linea</th><th>COD_EDIF</th><th>NUM_APTO</th><th>Propietario</th><th>Pago Total</th><th>Fecha Fact</th><th>Meses</th></tr></thead><tbody>
                @foreach($previewRows as $row)
                <tr><td class="text-xs">{{ $row['line'] }}</td><td class="text-xs">{{ $row['display']['cod_edif'] }}</td><td class="font-medium text-sm">{{ $row['display']['num_apto'] }}</td><td class="text-xs">{{ \Illuminate\Support\Str::limit($row['display']['propietario'], 25) }}</td><td class="text-xs font-semibold">{{ $row['display']['pago_total'] }}</td><td class="text-xs">{{ $row['display']['fecha_fact'] }}</td><td class="text-xs">{{ $row['display']['mes_deuda'] }}</td></tr>
                @endforeach
            </tbody></table></div></div>
        </div>
    ...
    <div class="space-y-6">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('financiero.movfactapto.importar.preview') }}" method="POST" enctype="multipart/form-data">
                    @csrf
