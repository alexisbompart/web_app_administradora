<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-heading font-bold text-navy-800">Catalogo de Gastos</h2>
                <p class="text-sm text-slate_custom-400 mt-1">cond_gastos</p>
            </div>
            <div class="flex items-center gap-3">
                <a href="{{ route('financiero.gastos.importar') }}" class="btn-primary">
        ...
        <div class="stat-card">
            <div class="flex items-center justify-between">
                ...
                <div class="w-10 h-10 bg-navy-800/10 rounded-lg flex items-center justify-center">
                    <i class="fas fa-receipt text-navy-800"></i>
                </div>
            </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="text-sm font-heading font-semibold text-navy-800">
                <i class="fas fa-receipt mr-2 text-burgundy-800"></i>Listado
            </h3>
        </div>
        <div class="card-body p-0">
            ...
            <div class="overflow-x-auto">
                <table class="table-custom">
                    ...
                    <tbody>
                        ...
                        <tr>
                                <td class="text-xs font-semibold">{{ $item->cod_gasto ?? $item->codigo }}</td>
                                <td class="text-xs">{{ $item->descripcion }}</td>
                                <td class="text-xs">{{ $item->tipo_gasto }}</td>
                                <td class="text-xs">{{ $item->clasificacion }}</td>
                                <td>
                                    @if($item->activo)<span class="badge-success text-xs">Activo</span>
                                    @else<span class="badge-danger text-xs">Inactivo</span>@endif
                                </td>
                        </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="p-8 text-center">
                <div class="w-16 h-16 bg-slate_custom-100 rounded-full flex items-center justify-center mx-auto mb-4">
                    <i class="fas fa-receipt text-2xl text-slate_custom-400"></i>
                </div>
