<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate_custom-400 mt-1">Carga completa — reemplaza todos los abonos existentes</p>
            </div>
    ...
    <div class="space-y-6">
        <div class="bg-amber-50 border border-amber-300 text-amber-800 px-5 py-4 rounded-xl">
            <div class="flex items-start gap-3"><i class="fas fa-exclamation-triangle text-xl mt-0.5"></i><div><p class="font-heading font-bold">Carga Completa — Se reemplazaran TODOS los abonos</p><p class="text-sm mt-1">Se eliminaran <strong>{{ number_format($summary['total_actual_bd']) }}</strong> abonos actuales y se insertaran <strong>{{ number_format($summary['validas']) }}</strong> nuevos.</p></div></div>
        </div>
        ...
        <form action="{{ route('financiero.abonos.importar.execute') }}" method="POST" onsubmit="return confirm('Se eliminaran {{ number_format($summary['total_actual_bd']) }} abonos actuales y se insertaran {{ number_format($summary['validas']) }} nuevos. Continuar?')">
            @csrf
            ...
            <div class="card-body">
                <form action="{{ route('financiero.abonos.importar.preview') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="max-w-xl mx-auto text-center">
                        ...
                        <h4 class="text-lg font-heading font-bold text-navy-800 mb-2">Carga Completa de Abonos</h4>
                        <p class="text-sm text-slate_custom-400 mb-2">Reemplaza <strong class="text-red-600">todos los abonos actuales</strong> con los del archivo.</p>
