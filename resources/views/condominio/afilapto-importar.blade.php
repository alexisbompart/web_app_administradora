<x-app-layout>
    ...
    <div class="space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div class="stat-card"><div class="flex items-center justify-between"><div class="stat-label">Anteriores Eliminados</div><div class="w-10 h-10 bg-slate_custom-100 rounded-lg flex items-center justify-center"><i class="fas fa-database text-slate_custom-500"></i></div></div><div class="stat-value text-amber-600">{{ number_format($results['previous_count']) }}</div></div>
        ...
        <div class="flex gap-3">
            <a href="{{ route('condominio.afilapto.importar') }}" class="btn-secondary"><i class="fas fa-file-import mr-2"></i>Nueva Carga</a>
    ...
    <div class="space-y-6">
        <div class="bg-amber-50 border border-amber-300 text-amber-800 px-5 py-4 rounded-xl">
            <div class="flex items-start gap-3"><i class="fas fa-exclamation-triangle text-xl mt-0.5"></i><div><p class="font-heading font-bold">Carga Completa — Se reemplazaran TODAS las afiliaciones</p><p class="text-sm mt-1">Se eliminaran <strong>{{ number_format($summary['total_actual_bd']) }}</strong> afiliaciones actuales y se insertaran <strong>{{ number_format($summary['validas']) }}</strong> nuevas.</p></div></div>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body p-0"><div class="overflow-x-auto"><table class="table-custom">
                <thead><tr><th>Linea</th><th>ID</th><th>PINT</th><th>Edificio</th><th>Apto</th><th>Comp</th><th>Estatus</th><th>Fecha</th><th>Notas</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-xs text-amber-600">{{ implode('; ', $row['display']['warnings'] ?? []) }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table></div></div>
        ...
        <form action="{{ route('condominio.afilapto.importar.execute') }}" method="POST" onsubmit="return confirm('Se eliminaran {{ number_format($summary['total_actual_bd']) }} afiliaciones actuales y se insertaran {{ number_format($summary['validas']) }} nuevas. Continuar?')">
            @csrf
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('condominio.afilapto.importar') }}" class="btn-secondary"><i class="fas fa-times mr-2"></i>Cancelar</a>
                <button type="submit" class="btn-primary" onclick="this.disabled=true; this.innerHTML='<i class=\'fas fa-spinner fa-spin mr-2\'></i>Procesando...'; this.form.submit();"><i class="fas fa-sync-alt mr-2"></i>Reemplazar Afiliaciones ({{ number_format($summary['validas']) }})</button>
            </div>
        </form>
        @endif
    </div>
