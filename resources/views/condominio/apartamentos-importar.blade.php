<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-heading font-bold text-navy-800">Importar Apartamentos</h2>
                <p class="text-sm text-slate_custom-400 mt-1">Carga masiva de apartamentos desde archivo pipe-delimited</p>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="flex gap-3">
            <a href="{{ route('condominio.apartamentos.index') }}" class="btn-primary">
                <i class="fas fa-list mr-2"></i>Ver Listado de Apartamentos
            </a>
    ...
    <div class="space-y-6">
                    ...
                    <table class="table-custom">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ $row['status'] === 'error' ? 'bg-red-50' : '' }}">
                                ...
                                <td class="text-xs">{{ $row['display']['estatus'] ?? '--' }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <form action="{{ route('condominio.apartamentos.importar.execute') }}" method="POST">
            @csrf
            <div class="card">
                <div class="card-header">
                    <h3 class="text-sm font-heading font-semibold text-navy-800">
                ...
                <div class="card-body">
                    @if($summary['update'] > 0)
                    <div class="mb-6">
                        <p class="text-sm font-semibold text-navy-800 mb-3">Se encontraron {{ $summary['update'] }} apartamentos que ya existen. Que desea hacer?</p>
                        <div class="flex flex-col sm:flex-row gap-4">
                            <label class="flex items-center gap-3 p-4 border border-slate_custom-200 rounded-xl cursor-pointer hover:bg-slate_custom-50 transition">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate_custom-400">Sobreescribir los datos de los apartamentos que ya existen</p>
                                </div>
                            </label>
                            <label class="flex items-center gap-3 p-4 border border-slate_custom-200 rounded-xl cursor-pointer hover:bg-slate_custom-50 transition">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate_custom-400">Solo importar los apartamentos nuevos</p>
                                </div>
                            </label>
                        </div>
                    </div>
        ...
        <div class="bg-yellow-50 border border-yellow-200 text-yellow-700 px-4 py-3 rounded-xl flex items-center gap-2">
            ...
            No hay filas validas para importar. Revise el archivo y los codigos de edificio.
        </div>
        <a href="{{ route('condominio.apartamentos.importar') }}" class="btn-secondary mt-4">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('condominio.apartamentos.importar.preview') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="max-w-xl mx-auto text-center">
                    ...
                    <h4 class="text-lg font-heading font-bold text-navy-800 mb-2">Seleccione el archivo de apartamentos</h4>
            ...
            <div class="mt-8 pt-6 border-t border-slate_custom-200">
                ...
                <div class="bg-slate_custom-50 rounded-xl p-4 overflow-x-auto">
                    <p class="text-xs text-slate_custom-500 mb-2">Header (primera linea):</p>
                    <code class="text-xs text-navy-800 break-all">''||A.ADMINISTRADO|ALICUOTA|...|NUM_APTO|...|A.TIPO_PAGO||''</code>
                    <p class="text-xs text-slate_custom-500 mt-3 mb-2">Datos (separados por <code>|</code>):</p>
                    <code class="text-xs text-navy-800 break-all">0|3.588||||SI|04129646380||825|...|003A|...</code>
                </div>
                <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div class="flex items-start gap-2 text-xs text-slate_custom-500">
                        <i class="fas fa-check-circle text-green-500 mt-0.5"></i>
                        <span><strong>COD_EDIF</strong> debe existir en la tabla de edificios</span>
                    </div>
                    <div class="flex items-start gap-2 text-xs text-slate_custom-500">
                        <i class="fas fa-check-circle text-green-500 mt-0.5"></i>
                        <span><strong>NUM_APTO</strong> es obligatorio para cada fila</span>
                    </div>
                    <div class="flex items-start gap-2 text-xs text-slate_custom-500">
                        <i class="fas fa-check-circle text-green-500 mt-0.5"></i>
                        <span>Duplicados se detectan por <strong>COD_EDIF + NUM_APTO</strong></span>
                    </div>
                    <div class="flex items-start gap-2 text-xs text-slate_custom-500">
                        <i class="fas fa-check-circle text-green-500 mt-0.5"></i>
